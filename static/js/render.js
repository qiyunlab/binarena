...
function exportSVG(mo) {
  ...
  const rena = mo.rena,
        w = rena.width,
        h = rena.height;
  svg.push(`<svg version="1.1" width="${w}" height="${h}" ` +
  ...
  for (let i = 0; i < n; i++) {
    ...
    // determine x- and y-coordinates
    // skip contigs outside viewport
    x = X[i] * scaleX + offsetX;
    if (x + rad < 0 || x - rad > w) continue;
    y = Y[i] * scaleY + offsetY;
    if (y + rad < 0 || y - rad > h) continue;
    ...
    // highlight circle
    hi = high[i];
    if (hi) highs[hi - 1].push([x, y, rad + hirad]);
  }
  svg.push('  </g>');

  // render highlights
  svg.push('  <g id="highlight">');
  let hs, m;
  for (let i = 0; i < nhigh; i++) {
    hs = highs[i];
    m = hs.length;
    if (!m) continue;
    svg.push(`    <g id="highight-${i + 1}" ` +
      `fill="${HIGHLIGHT_PALETTE[i]}" ` +
      'fill-opacity="0.4">');
    for (j = 0; j < m; j++) {
      [x, y, rad] = hs[j];
      svg.push('      <circle ' +
